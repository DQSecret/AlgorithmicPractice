package adventofcode.y2015.d3

import java.awt.Point

/**
 * 分析完题目,感觉是一个计算二维范围的问题
 *
 * @author DQ For Olivia
 * @since 2021/9/18 12:13 下午
 * @see <a href="https://adventofcode.com/2015/day/3">文章</a>
 */
class PerfectlySphericalHousesInAVacuum {

    companion object {
        const val puzzle: String =
    ...
    /**
     * 二维坐标
     * 切记 Map 的 Key 不能使用 hashcode 太容易哈希冲突了
     */
    fun part1(input: String = puzzle): Int {
        ...
        map[current.toString()] = current
        // 循环
        input.forEach {
            when (it) {
                Up.action -> current.translate(0, -1)
                Down.action -> current.translate(0, 1)
                Left.action -> current.translate(-1, 0)
                Right.action -> current.translate(1, 0)
            }
            map[current.toString()] = current
        }
        return map.size
    ...
    fun part2(input: String = puzzle): Int {
        ...
        val map = mutableMapOf<String, Point>()
        // 填充初始坐标
        val current1 = Point(0, 0)
        map[current1.toString()] = current1
        val current2 = Point(0, 0)
        map[current2.toString()] = current2
        // 循环
        input.forEachIndexed { index, c ->
            val role = if (index % 2 == 1) current1 else current2
            when (c) {
                Up.action -> role.translate(0, -1)
                Down.action -> role.translate(0, 1)
                Left.action -> role.translate(-1, 0)
                Right.action -> role.translate(1, 0)
            }
...
sealed class Direction(val action: Char)
object Up : Direction('^')
...
fun main() {
    PerfectlySphericalHousesInAVacuum().run {
        part1().also { println("至少收到一份礼物的房间的数量: $it") }
        part2().also { println("两条线路并行时,至少收到一份礼物的房间的数量: $it") }
    }
}