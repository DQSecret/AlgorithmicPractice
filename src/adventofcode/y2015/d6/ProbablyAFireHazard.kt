 ...
 * @since 2021/9/24 4:09 下午
 * @see <a href="https://adventofcode.com/2015/day/6">文章</a>
 */
class ProbablyAFireHazard {
    ...
    fun part1(input: List<String> = puzzle): Int {
        ...
        // 执行指令
        input.map { Action.transform(it) }
            .forEach { it.execute(matrix) }
        // 统计结果
        return matrix.sumBy {
            it.count { status ->
                status > 0
...
fun main() {
    ProbablyAFireHazard().run {
        part1().also { println("最后亮着的灯的数量为: $it") }
        part2().also { println("最后所有亮着的灯的总亮度为: $it") }
    }
}
...
sealed class Action {
    ...
    companion object {
        fun transform(desc: String): Action {
            val start: Point
            val end: Point
            desc.split(" ")
                .also { subDesc ->
                    subDesc[subDesc.size - 3]
                        .split(",")
                        .map { it.toInt() }
                        .let {
                            start = Point(it.first(), it.last())
                        }
                    subDesc.last()
                        .split(",")
                        .map { it.toInt() }
                        .let {
                            end = Point(it.first(), it.last())
                        }
                }
            return when {
                desc.startsWith(TurnOn.ACTION) -> TurnOn(start, end)
                desc.startsWith(TurnOff.ACTION) -> TurnOff(start, end)
                desc.startsWith(Toggle.ACTION) -> Toggle(start, end)
                else -> throw RuntimeException("指令异常,请检查输入指令是否合规: $desc")
            }
        }
    }

    abstract fun execute(matrix: Array<IntArray>)
    abstract fun execute2(matrix: Array<IntArray>)
}

class TurnOn(private val start: Point, private val end: Point) : Action() {

    companion object {
        const val ACTION = "turn on"
    }

    override fun execute(matrix: Array<IntArray>) {
    ...
    override fun execute2(matrix: Array<IntArray>) {
        for (x in start.x..end.x) {
            for (y in start.y..end.y)
                matrix[x][y] += 1
        }
...
class TurnOff(private val start: Point, private val end: Point) : Action() {

    companion object {
        const val ACTION = "turn off"
    }
...
class Toggle(private val start: Point, private val end: Point) : Action() {

    companion object {
        const val ACTION = "toggle"
    }
